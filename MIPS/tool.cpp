...
void Tool::pushSignOpera(char c){
    signs[signsSize].isOperator = 1;
    signs[signsSize].opera = c;
    signsSize++;
}

void Tool::pushSignNum(double n){
...
void Tool::deleteReverseItem(int pos){
    int i;
...
void Tool::insertReverseNum(int pos, double n){
    int i;
...
int Tool::isNumber(char c){
    return (c>='0'&&c<='9')||(c=='.');
}

int Tool::isOperator(char c){
    int flag = 0,i;
    for(i = 0; i<sizeof(operators); i++){
        if (c == operators[i]){
            flag = 1;
...
void Tool::pushOpera(char opera){//Operator stack
    operatorS[++operatorSTop] = opera;
}


int Tool::isNotEmptyOperaS(){
...
void Tool::replaceString(char s[], int pos, int len, char s1[]){//Replace the s from pos to len with s2
    int i;
    char s2[1000];
...
void Tool::tranString(char s[]){//Format string. For example "sin(3.14)+abs(-1)" is format to "J(3.14)+Y(-1)"
    int pos = 0;
    int i;
    while (pos < strlen(s)){
...
int Tool::getOperaArgNum(char op){//Get operator's number of arguments.
    int i;
    for (i = 0; i < sizeof(nameTran); ++i) {
        if(nameTran[i] == op){
            return nameArgNum[i];
...
int long Tool::fact(int n){//return the number's factor
    if (n < 0)
        return -1;
    if (n > 1)
        return fact(n - 1) * n;
...
double Tool::calculate(double *n, char op, int num){//Arguments are in *n. op is the operator. num is the number of arguments
    switch (op) {
    case ',':
        return n[num - 1];
    case '#':
...
void Tool::calculateOpera(char op, int pos){//Change the reverseSigns(stack) when calculating
